  ...
  renderTableItem = (item, index) => {
    return (
      <tr>
        ...
        <td><span className= "moreButton" onClick = { () => this.setState({ summary: item.summary, edit: false }) }>More</span><span className="editButton" onClick= { (event ) => this.validateEdit(event, item.subject, item.date, item.class, item.summary) } >Edit</span></td>
      </tr>
    );
  }
  checkEdit = () => {
